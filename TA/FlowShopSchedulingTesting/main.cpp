...
int main() {
	...
	for(double alfa = 0.30; alfa <= 0.50; alfa += 0.01) {
		cout << "Ejecutando GRASP con ALFA = " << alfa << endl;

		vector<int> mejorSecuencia = GRASP(tareas, numMaquinas, alfa);

		int posibleMejorMakespan = calcularMakespan(tareas, mejorSecuencia, numMaquinas);
		cout << "Para ALFA: " << alfa << " -> Mejor makespan: " << posibleMejorMakespan << endl << endl;

		if(posibleMejorMakespan < mejorMakespan) {
			mejorMakespan = posibleMejorMakespan;
			mejorAlfa = alfa;
		}
	}

	cout << "Luego de realizar las pruebas, se concluye lo siguiente:\n";
	cout << "Mejor ALFA: " << mejorAlfa << endl;
	cout << "Menor makespan obtenido: " << mejorMakespan << endl;
